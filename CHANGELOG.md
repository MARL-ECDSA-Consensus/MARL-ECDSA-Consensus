...
> **⚠️ 历史存档说明（2026-09-14 追加）**：本 CHANGELOG 按时间顺序记录历次变更，其中的实验数字（如旧版提升率、p 值、测试数、λ 取值）反映**当时**的配置与口径，**不代表现行申报口径**。现行权威口径请以竞赛交付物为准：3000 回合收敛验证 env_reward **+29.2%**（n=22，Welch **p=0.126 不显著**，d=0.47）；测试 **1574 passed / 1 skipped / 0 failed**；Nash 安全裕度 **50%**（λ=0.1）。
