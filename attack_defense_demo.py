...
def demo_observation_forgery() -> Dict:
    """
    攻击1: 观测伪造
    恶意智能体谎报自己的位置，诱使其他智能体避让，抢占最优路径

    无BC: 其他智能体无法验证位置真实性 → 被欺骗让路
    有BC: 位置数据需ECDSA签名 → 签名验证失败 → 拦截
    """
    logger.info("=" * 60)
    logger.info("攻击1: 观测伪造 (Observation Forgery)")
    logger.info("=" * 60)
    ...
    result["no_bc"] = {
        ...
        "description": "无法验证位置真实性，恶意智能体成功抢占路径",
        "forged_data": forged_position,
        "true_data": true_position,
    }

    # --- 有BC模式 ---
    logger.info("\n[有BC] 位置数据必须ECDSA签名后广播")

    # 恶意智能体用自己的私钥对伪造位置签名
    forged_msg = json.dumps(forged_position, sort_keys=True).encode()
    signature = _sign_message(km, "malicious_agent", forged_msg)

    # 验证方检查：签名是否对应声明的agent_id（验证通过，因为确实是用自己的私钥签的）
    verified = _verify_message(km, "malicious_agent", forged_msg, signature)
    logger.info(f"  签名验证（malicious_agent自身）: {'通过' if verified else '失败'}")

    # 关键攻击：攻击者试图冒充honest_agent
    # 用malicious_agent的私钥签名，但声明agent_id=honest_agent
    forged_msg2 = json.dumps(
        {"x": 0.5, "y": 0.5, "agent_id": "honest_agent"}, sort_keys=True
    ).encode()
    fake_signature = _sign_message(km, "malicious_agent", forged_msg2)
    # 验证方用honest_agent的公钥验证 → 失败（签名密钥不匹配）
    verified2 = _verify_message(km, "honest_agent", forged_msg2, fake_signature)
    logger.info(f"  冒充honest_agent签名验证: {'通过' if verified2 else '失败（拦截）'}")

    # 链上存证验证：构造交易并上链
    bc = Blockchain()
    sig_hex = signature.hex()
    tx = _make_tx("malicious_agent", true_position, nonce=1,
                  signature_hex=sig_hex, tx_type="position_report")
    ...
    block = Block(
        ...
        transactions=[tx],
        state_root=hashlib.sha256(json.dumps(true_position, sort_keys=True).encode()).hexdigest(),
    )
    bc.append_block(block)
    logger.info(f"  链上存证: Block #{block.block_height}, hash={block.block_hash[:16]}...")

    # 篡改检测：修改链上数据
    tampered_data = dict(true_position)
    tampered_data["position"] = forged_position
    ...
    logger.info(f"[有BC] 攻击被拦截！签名验证+链上哈希校验双重防护")
    result["with_bc"] = {
        "attack_successful": False,
        "description": "ECDSA签名验证+链上哈希校验，伪造数据被拦截",
        "signature_valid": verified,
        "impersonation_blocked": not verified2,
        "chain_tamper_detected": not hash_match,
        "block_index": block.block_height,
    }
...
def generate_report(results: List[Dict]):
    ...
    report_path = Path(__file__).parent / 'results' / 'attack_defense_report.json'
