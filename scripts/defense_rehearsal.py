...
QUIZ_BANK = [
    ...
    ("为什么500回合和3000回合的BC提升差距这么大？", "500回合agent未充分收敛，BC激励的长期塑形效果未显现（+13.4%）；3000回合收敛后回升至+29.2%（22种子/组口径）。这说明BC激励是长期协作塑形信号，需充分训练才能体现。这是科学发现，不是数据矛盾。"),
    ...
    ("测试覆盖了多少用例？", "1574个测试用例全部通过，覆盖共识/密码学/MARL/网络/Dashboard全模块。使用pytest框架，CI流水线持续验证。"),
]
